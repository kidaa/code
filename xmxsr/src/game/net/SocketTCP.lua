...
local funList_ = {}
function SocketTCP:addEventListener( __key, _fun )
	funList_[__key] = _fun
	return self
end
function SocketTCP:dispatchEvent( _event )
	funList_[_event.name](_event)
	return self
end
...
function SocketTCP:send(__data)
	...
	local len = self.tcp:send(__data)
	print("-------send:",len,"  data:",__data)
    return len
end
